      ...
      <form onSubmit={handleSubmit} className="relative max-w-4xl mx-auto">
        <div className="relative group">
          <input
            ...
            className="w-full p-4 md:p-5 pr-14 md:pr-16 rounded-3xl bg-white/80 backdrop-blur-md text-red-800 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-red-800/50 focus:border-red-800 transition-all duration-200 shadow-lg shadow-gray-200/50 placeholder-gray-500 group-hover:shadow-xl group-hover:shadow-gray-200/60"
            disabled={isLoading}
            autoFocus
          />
          <button
        ...
        {!input && !isLoading && (
          <div className="flex flex-wrap gap-2 mt-4 justify-center">
            <button
              type="button"
              onClick={() => setInput("What courses do I need for my major?")}
            ...
            <button
              type="button"
              onClick={() => setInput("How do I plan my schedule?")}
              className="px-4 py-2 bg-white/60 hover:bg-white/80 backdrop-blur-sm border border-gray-200 rounded-full text-sm text-gray-600 hover:text-gray-800 transition-all duration-200 hover:shadow-md hover:scale-105"
            >
              Schedule Planning
            </button>
            <button
              type="button"
              onClick={() => setInput("What are the graduation requirements?")}
              className="px-4 py-2 bg-white/60 hover:bg-white/80 backdrop-blur-sm border border-gray-200 rounded-full text-sm text-gray-600 hover:text-gray-800 transition-all duration-200 hover:shadow-md hover:scale-105"
            >
              Graduation Info
            </button>
